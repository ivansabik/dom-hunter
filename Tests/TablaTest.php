...
class TablaTest extends PHPUnit_Framework_TestCase {
    ...
    function testNavegacion() {
        $tabla = new Tabla();
        $tabla->dom_path(array('getElementById' => '3208544064715720055515HIS', 'nextSibling' => '', 'lastChild' => ''));
        $tabla->columns(array('fecha', 'lugar_movimiento', 'comentarios'));
        $this->assertEquals(self::$_tabla_estafeta, $tabla->duckTest($this->_dom));
    }

    # Modificado el original de Estafeta para probar buscar por id del DOM

    function testIdNodo() {
        $tabla = new Tabla();
        $tabla->dom_id('3208544064715720055515MOV');
        $tabla->columns(array('fecha', 'lugar_movimiento', 'comentarios'));
        $this->assertEquals(self::$_tabla_estafeta, $tabla->duckTest($this->_dom));
    }

    function testNumeroOcurrencia() {
        $tabla = new Tabla();
        $tabla->result_num(1);
    ...
    function testNumOcurrenciaInvalido() {
        $this->markTestIncomplete('testNumOcurrenciaInvalido() no esta definido');
    }

    function testResultadoQueryNoTable() {
